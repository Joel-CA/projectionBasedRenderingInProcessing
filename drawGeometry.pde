...
MeshBuilder.Plane frontPlane = new MeshBuilder.Plane(new float[][] {{0, 0, 0}}, Z_MINUS);
MeshBuilder.Plane backPlane = new MeshBuilder.Plane(new float[][] {{0, 0, 100}}, Z_PLUS);
final MeshBuilder.Plane[] FRUSTUM = new MeshBuilder.Plane[] {frontPlane};

//Function to clip polygon against the viewing frustum
List<float[]> frustumClipPolygon(float[][] polygonPoints) {
  //Clip the face against each plane in the frustum
  List<float[]> outputList = new ArrayList<>(Arrays.asList(polygonPoints));
  for (MeshBuilder.Plane frustrumPlane : FRUSTUM) {
    List<float[]> inputList = outputList;
    outputList.clear();
